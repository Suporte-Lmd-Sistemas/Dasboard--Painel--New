import Topbar from "../components/Topbar";
<<<<<<< HEAD
import ListaRelatorios from "../components/ListaRelatorios.jsx";
import "../styles/topbar.css";

function RelatoriosConsultoria({ onToggleSidebar, isMobileOrTablet }) {
  return (
    <>
      <Topbar titulo="Relatórios" caminho="Dashboard / Consultoria / Contabilidade" onToggleSidebar={onToggleSidebar}
        isMobileOrTablet={isMobileOrTablet} />
      <ListaRelatorios
        categoria="consultoria"
        titulo="Relatórios de Consultoria / Contabilidade"
      />
    </>
=======
import "../styles/relatorios.css";
import "../styles/topbar.css";

function RelatoriosConsultoria() {
  const relatorios = [
    { id: 1, nome: "DRE gerencial", periodo: "Últimos 7 Dias", data: "Hoje" },
    { id: 2, nome: "Notas fiscais emitidas", periodo: "Agosto", data: "Hoje" },
    { id: 3, nome: "Notas fiscais recebidas", periodo: "Mês Passado", data: "Hoje" },
    { id: 4, nome: "Apuração contábil", periodo: "Setembro", data: "Ontem" },
    { id: 5, nome: "Resultado por competência", periodo: "Últimos 30 Dias", data: "Ontem" },
    { id: 6, nome: "Balancete resumido", periodo: "Últimos 7 Dias", data: "22/04" },
    { id: 7, nome: "Conciliações", periodo: "Últimos 7 Dias", data: "14/02" },
    { id: 8, nome: "Movimentação fiscal", periodo: "Últimos 7 Dias", data: "10/02" },
    { id: 9, nome: "Resumo tributário", periodo: "Últimos 7 Dias", data: "22/04" },
    { id: 10, nome: "Demonstrativo de despesas", periodo: "Últimos 7 Dias", data: "14/02" },
    { id: 11, nome: "Demonstrativo de receitas", periodo: "Últimos 7 Dias", data: "10/02" },
    { id: 12, nome: "Análise contábil consolidada", periodo: "Últimos 7 Dias", data: "22/04" },
  ];

  return (
    <div className="relatorio-list-page">
      <Topbar titulo="Relatórios" caminho="Dashboard / Contabilidade" />

      <div className="relatorio-filter-bar">
        <div className="empresa-chip-box">
          <span>Empresa:</span>
          <button type="button" className="empresa-chip">
            0 - Todas as Empresas
          </button>
        </div>
      </div>

      <div className="relatorio-list-box">
        <div className="relatorio-list-header">
          <h2>Relatórios Contabilidade</h2>
        </div>

        <div className="relatorio-list-table-responsive">
          <table className="relatorio-list-table">
            <thead>
              <tr>
                <th>Relatório</th>
                <th>Período</th>
                <th>Data</th>
                <th>Ações</th>
              </tr>
            </thead>

            <tbody>
              {relatorios.map((item) => (
                <tr key={item.id}>
                  <td>{item.nome}</td>
                  <td>{item.periodo}</td>
                  <td>{item.data}</td>
                  <td>
                    <div className="acoes-relatorio">
                      <button type="button" className="acao-icon-btn" title="Visualizar">
                        👁
                      </button>
                      <button type="button" className="acao-icon-btn" title="Exportar">
                        ⬇
                      </button>
                      <button type="button" className="acao-pdf-btn">
                        PDF
                      </button>
                    </div>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </div>
    </div>
>>>>>>> c57a7228d900d0db50b76e941e71cd9a3d700f4f
  );
}

export default RelatoriosConsultoria;