...
def get_engine_for_cnpj(cnpj: str):
    ...
    if not company:
        if not companies:
            raise Exception("Nenhuma empresa cadastrada no companies.json")
        company = companies[0]
        # print(f"DEBUG: CNPJ '{cnpj_clean}' nao encontrado ou default. Usando {company['cnpj']}")
    else:
        # print(f"DEBUG: Conectando base de {company['razao_social']} ({cnpj_clean})")
        pass
